...
function Banner() {
  ...
  useEffect(() => {
    const words = ["Kitchen", "Bathroom", "Interior"];
    ...
    const timeout = setTimeout(() => {
      if (!isDeleting) {
        ...
        if (displayText === currentWord) {
          setTimeout(() => setIsDeleting(true), 5000); // wait 5s after typing
        }
      } else {
        setDisplayText(currentWord.substring(0, displayText.length - 1));
    ...
    <div className="relative inset-0 w-full h-full">
      ...
      <div className="w-full h-full absolute top-0 z-20 flex flex-col justify-between">
        ...
        <div className="w-full h-full flex px-3 flex-col gap-3 items-center justify-center text-white">
          ...
          <button onClick={() => navigate("/products")} className="relative border bg-white/15 mt-[2%] border-white/30 backdrop-blur-[1px]">
            Explore Products
          </button>
        </div>
      </div>
